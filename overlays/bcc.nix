...
with super;
{
  bcc = stdenv.mkDerivation rec {
    ...
    buildInputs = [ makeWrapper cmake llvmPackages.llvm llvmPackages.clang-unwrapped linux
    flex bison elfutils python pythonPackages.netaddr luajit netperf iperf
  ];

  cmakeFlags="-DBCC_KERNEL_MODULES_DIR=${linux.dev}/lib/modules -DBCC_KERNEL_HAS_SOURCE_DIR=1";
